...
def add_expenditure():
    ...
    if not expense_list.strip():
        print("-" * 50 + "\n")
        print("Expense cannot be empty. Please enter a valid number.")
        return
...
def edit_expense():
    ...
    try:
        ...
        if 0 <= i < len(expenditure):
            ...
            print("\n" + "-" * 50)
            print(
                f"Invalid amount: '{expense}' is out of range please enter a valid integer between 1 and {len(expenditure)}")
            print("-" * 50 + "\n")
    except ValueError:
        print("\n" + "*" * 50)
...
def delete_expense():
    ...
    if 0 <= expenditure_to_delete - 1 < len(expenditure):
        ...
        save_expenditure(expenditure)
        print(f"expense {expenditure_to_delete} ('{deleted_expense['expense']}') has been removed -\n")
        x = datetime.datetime.now()
        print(x.strftime("%c"))
    else:
        print("\n" + "-" * 50)
        print(
            f"Invalid number: '{expenditure_to_delete}' is out of range. Please enter a valid number between 1 and {len(expenditure)}.")
        print("-" * 50 + "\n")
        return
